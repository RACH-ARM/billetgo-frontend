            ...
            <div className="flex gap-3 mt-4">
              {SOCIALS.map(({ label, href, color, icon }) => (
                <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label}
                  className={`w-9 h-9 rounded-xl bg-bg-card border border-white/10 flex items-center justify-center text-white/40 transition-all ${color} hover:border-white/30`}>
                  {icon}
                </a>
              ))}
            </div>
